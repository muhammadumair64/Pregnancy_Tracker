...
//                preferencesManager!!.put(PreferencesManager.Key.IS_APP_ADS_FREE, true)
    ...
    fun makeAppPremium() {
        ...
        MyApplication.mInstance.preferenceManager.put(
            PreferencesManager.Key.IS_APP_ADS_FREE,
            true
        )
        MyApplication.mInstance.preferenceManager.put(
            PreferencesManager.Key.IS_APP_PREMIUM,
            true
        )
        Toast.makeText(
            MyApplication.mInstance,
        ...
        try{
            ...
            billingClient?.queryProductDetailsAsync(
                ...
                if(prodDetailsList.isNotEmpty()){
                    ...
                    oneTimeProductPremiumPrice = prodDetailsList[0].subscriptionOfferDetails?.get(0)?.pricingPhases?.pricingPhaseList?.get(0)?.formattedPrice.toString()
                    Log.d(TAG,"price is $oneTimeProductPremiumPrice" )
                }
            }
        }
        catch (e:Exception){
