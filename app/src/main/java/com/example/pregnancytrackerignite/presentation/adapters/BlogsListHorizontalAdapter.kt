...
import com.example.pregnancytrackerignite.databinding.ItemBlogsHorizontalRvBinding

class BlogsListHorizontalAdapter(
    private val context: Context,
    private val item: ArrayList<MainBlogModel>,
    private val onItemClicked: (MainBlogModel) -> Unit
) : RecyclerView.Adapter<BlogsListHorizontalAdapter.BlogsViewHolder>() {
    private var selectedPosition = -1

    class BlogsViewHolder(val binding: ItemBlogsHorizontalRvBinding) :
        RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BlogsViewHolder {
        val binding = ItemBlogsHorizontalRvBinding.inflate(LayoutInflater.from(parent.context), parent, false)
